...
const StdLocation = Component(
    React.createClass({
        getInitialState() {
            ...
            return {
                lat:
                    fs.data && fs.data[latName || p.name + "Lat"]
                        ? fs.data[latName || p.name + "Lat"]
                        : "",
                lng:
                    fs.data && fs.data[lngName || p.name + "Lng"]
                        ? fs.data[lngName || p.name + "Lng"]
                        : ""
            };
        },
        marker: null,
        ...
        componentDidMount: function() {
            ...
            mapsapi().then(maps => {
                ...
                this.setState({ mapType: this.MapTypes.ROADMAP });
                            ...
                            Geocode.go({
                                ...
                                success(r) {
                                    if (r.results && r.results.length)
                                        _this.refs.placeSuggest.child.setSearchText(
                                            r.results[0].formatted_address
                                        );
                                    else
                                        emitter.emit(
                                            "info_msg",
                                            "Could not geocode a textual address"
                                        );
                                }
                            });
                        }
                ...
                var idleListener = google.maps.event.addListener(
                    ...
                    function(event) {
                        if (
                            fs.data &&
                            fs.data[
                                p.field.options.latName || p.name + "Lat"
                            ] &&
                            fs.data[p.field.options.lngName || p.name + "Lng"]
                        ) {
                            var latLng = {
                                lat: Number(
                                    fs.data[
                                        p.field.options.latName ||
                                            p.name + "Lat"
                                    ]
                                ),
                                lng: Number(
                                    fs.data[
                                        p.field.options.lngName ||
                                            p.name + "Lng"
                                    ]
                                )
                            };
